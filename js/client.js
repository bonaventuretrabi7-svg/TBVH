...
/* ── Le bandeau (solde/cadenas) utilise position:sticky (voir
   css/style.css, .pgc-hero-card) plutôt que fixed : modèle volontairement
   simple, entièrement géré par le navigateur — corrige un bug persistant
   (bandeau/QR bloqués après un défilement bas puis retour en haut, sur
   WebView Android/iOS) qui survivait à toute tentative de correctif sur
   l'ancien modèle position:fixed + réserve d'espace calculée en JS +
   animation de compactage au défilement. Plus rien à synchroniser ici :
   sticky réserve sa propre place dans le flux du document tout seul, la
   carte QR (flux normal juste après) suit sans aucun JS. */
